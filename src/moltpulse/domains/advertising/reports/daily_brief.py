...
class DailyBriefReport(DailyBriefGenerator):
    ...
    def _create_thought_leaders_section(
        ...
        """Create curated thought leader highlights section."""
        # Build lookup for thought leader metadata from profile
        leader_notes = {}
        for leader in self.profile.thought_leaders:
        ...
        for item in items:
            handle = item.author_handle.lower().lstrip("@")
            leader_info = leader_notes.get(handle, {})
            ...
            formatted_items.append(
                {
                    "author_handle": f"@{item.author_handle}",
                    "author_name": leader_info.get("name", item.author_name),
                    "author_notes": leader_info.get("notes", ""),
                    "text": item.text,  # Full text, not truncated
                    "url": item.url,
        ...
        return schema.ReportSection(
            title="Thought Leader Highlights",
            items=formatted_items,
        )
